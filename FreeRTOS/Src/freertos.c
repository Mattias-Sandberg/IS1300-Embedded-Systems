...
/*In FreeRTOS, semaphores are synchronization mechanisms that enable tasks to coordinate
 * access to shared resources by controlling their execution based on specific conditions,
 * allowing tasks to safely interact and prevent race conditions (unpredictable).*/
#include "semphr.h"
...
//uint8_t varBlink2 = 0;
//uint8_t varBlink3 = 0;
SemaphoreHandle_t bSem = NULL;  //Declaration of the semaphore
...
void MX_FREERTOS_Init(void) {
  ...
  /* USER CODE END RTOS_MUTEX */
	bSem = xSemaphoreCreateBinary(); // Creating a binary semaphore
...
void Blink1(void *argument)
{
	...
	for(;;)
	{
		if(bSem != NULL) // Check if the semaphore handle is valid (not NULL)
		{
			// Attempt to take the semaphore with an indefinite timeout
			if( xSemaphoreTake( bSem, portMAX_DELAY ) == pdTRUE )
			{
				// Semaphore successfully taken, Toggle the state of the LD2 pin
				HAL_GPIO_TogglePin(GPIOA, LD2_Pin);		//Makes LED2 Blink

				// Release the semaphore since its purpose is fulfilled
				xSemaphoreGive(bSem);
			}
		}
...
void Userbutton(void *argument)
{
	...
	for(;;)
	{
		if(bSem != NULL)
		{
			 // Check if the semaphore handle xSem is valid (not NULL)
			if (HAL_GPIO_ReadPin(B1_GPIO_Port, B1_Pin)==GPIO_PIN_RESET &&
					HAL_GPIO_ReadPin(GPIOA, LD2_Pin)==GPIO_PIN_RESET)
			{
				 // Check conditions for taking the semaphore:
				    // 1. B1 button is pressed (GPIO_PIN_RESET)
				    // 2. LD2 pin is in the reset state when its off. (GPIO_PIN_RESET)

				 // Attempt to take the semaphore with a timeout of 10 system ticks
					xSemaphoreTake(bSem, 10);
			} else
			{
				// Conditions for taking the semaphore not met, release (give back) the semaphore
				xSemaphoreGive(bSem);
				//This ensures that the semaphore is available for other tasks
				//if the conditions for taking it are not met.
			}
		}
		vTaskDelayUntil( &xLastWakeTime, xPeriod );
	}
